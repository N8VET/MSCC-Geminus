...
int Calibrate_Si5351_Set_PPM(uint32_t frequency) {//This is called by Process_Frequency_Calibration()
    ...
    printf("[%d] Calibrate_Si5351_Set_PPM. total_int_part: %d,total_dec_part: %d, Delta: %d\n", line_number++,
...
int main(int argc, char **argv) {
    ...
    if (status >= 0) {
        ...
        printf("RESET CALIBRATION TO DEFAULT VALUE Y/N (ANY OTHER KEY TO QUIT): ");
        ret = getchar();
        ret = toupper(ret);
        if (ret == 'Y') {
            Freq_Set_Transceiver_Calibration(G_int, G_dec);
            printf("THE CALIBRATION HAS BEEN RESET \n");
        } else if (ret == 'N') {
            //Freq_Set_Transceiver_Calibration(G_int, G_dec);
            ...
            if (G_dial_freq >= 1) {
                ...
                printf("THE TRANSCEIVER HAS BEEN CALIBRATED \n");
            }
        } else {
            printf("CALIBRATION HAS NOT BEEN RUN \n");
        }
    } else {
