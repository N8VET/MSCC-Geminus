    ...
    for (bus = usb_get_busses(); bus; bus = bus->next) {
        for (dev = bus->devices; dev; dev = dev->next) {
            if (dev->descriptor.idVendor == vid && dev->descriptor.idProduct == pid) {
                //print_time(0);
                fprintf(stdout, "srOpen -> Transceiver FOUND\n");
                return_status = 0;
                            ...
                            if (error = usb_claim_interface(usbHandle, 0) < 0) {
                                ...
                                *status = usbGetStringAscii(srUsbHandle, dev->descriptor.iSerialNumber, 0x0409, G_serial_number, 
                                ...
                                if (*status > 0) {
                                    ...
                                    printf("srOpen -> G_Usb_serial_number: %s, status: %d\n",
                                        G_Usb_serial_number, *status);
                                }
                            }
                        }
                    }
...
int init_ini() {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        ...
        strcat(l_path, "/.local/share/mscc/Multus_mscc.ini");
...
int RPI_Init() {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/mscc-rpi.ini");
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
            fprintf(Multus_ini, "PROFICIO_SERIAL_NUMBER=%s;\n", G_Usb_serial_number);
            fprintf(Multus_ini, "MSCC_PORT=%d;\n", G_mscc_port);
...
int Create_PA_i2c() {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/i2c.ini");
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
...
int Create_Fortis_I2C(int mfc, int meter,int mfc_serial_port) {
    int status = 0;
    FILE *Multus_ini;
    char l_path[MAX_PATH] = {0};
    char directry[MAX_PATH] = {0};
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/i2c.ini");
        _itoa(mfc_serial_port, comm_port_number, 10);
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
            fputs("G_MASTER_CONTROLLER_attached=2;\n", Multus_ini);
...
int Create_MSCC_PW_I2C(int transceiver_type,int mfc, int meter, int mfc_serial_port) {
    int status = 0;
    FILE* Multus_ini;
    char l_path[MAX_PATH] = { 0 };
    char directry[MAX_PATH] = { 0 };
...
void build_output_devices(int device_index) {
    ...
    if (index < MAX_INPUT_DEVICES) {
        ...
        if (name == NULL) {
            printf("Sound Playback (Speaker) Number: %d, name: %s \n",
                G_output_devices[index].device_index, G_output_devices[index].name);
        }
        index++;
...
void build_input_devices(int device_index) {
    ...
    if (index < MAX_INPUT_DEVICES) {
        ...
        if (name == NULL) {
            printf("Sound Recording (Microphone) device Number: %d, name: %s \n",
...
int Update_Speaker_Config(int record) {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/speaker-sound-device.ini");
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
...
int Update_Microphone_Config(int record) {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/microphone-sound-device.ini");
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
            _itoa(record, device_number, 10);
            fprintf(Multus_ini, "%s\n", device_number);
...
int Update_Serial_Config(int record) {
    ...
    if ((homedir = My_getenv("HOME")) != NULL) {
        strcpy(l_path, homedir);
        strcat(l_path, "/.local/share/mscc/comm-port.ini");
        Multus_ini = fopen(l_path, "w");
        if (Multus_ini != NULL) {
...
int Get_Serial_Ports() {
    ...
    for (i = 0; i < 256; i++) {
        ...
        if (hSerial != INVALID_HANDLE_VALUE) {
            CloseHandle(hSerial);
            comm_ports[count] = G_comm_port[0];
            count++;
...
int delete_trans_ini_file() {
    ...
    if (SUCCEEDED(hr)) {
        ...
        strcat(l_path, "\\MSCC-PW\\.local\\share\\mscc\\sdrcore_trans.ini");
#endif
...
int main(int argc, char** argv) {
    ...
    printf("AUDIO-RESET. DO YOU WISH TO RESET AUDIO DEVICE INITIALIZATION? ENTER 0 FOR NO, 1 FOR YES: ");
    scan_status = scanf("%hhd", &rego);
    if (rego == 1) {
        status = delete_trans_ini_file();
        status = delete_recv_ini_file();
    }
    printf("AUDIO DEVICE INITIALIZATION COMPLETE");
